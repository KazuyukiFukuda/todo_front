...
import { postTask } from "../lib/api/tasks";
...
const CreateTask: React.FC = () => {
  ...
  const [subtasks, setSubtasks] = useState<PostSubtask[]>([]);
  ...
  const assigneeEmailHandler = (event: SelectChangeEvent) => {
    setAssigneeEmail(event.target.value as string);
  };
  ...
  const subtaskDeleteHandler = async (deleteId: number) => {
    ...
    setSubtasks(subtasks.filter((subtask, index) => index !== deleteId));
  };
        ...
        >
          ...
          <Grid item xs={valueXS}>
            <FormControl fullWidth>
              <Select value={assigneeEmail} onChange={assigneeEmailHandler}>
                {userList.map((user) => (
                      ...
                      >
                        <Grid item>
                          <TextField
                            ...
                            id={`subtask${index}`}
                            autoFocus
                            value={subtasks[index].description}
                            onChange={(
                              e: React.ChangeEvent<HTMLInputElement>
                            ) => {
                              subtaskEditHandler(index, e.target.value);
                            }}
                          />
                        </Grid>
                        <Grid item>
